...
function hasRoute(name: string) {
  try {
    if ((window as any).route) return (window as any).route().has(name)
    return (ziggyRoute as any)().has(name)
  } catch { return false }
}
function href(name: string, params?: any, absolute?: boolean) {
  try { return ziggyRoute(name, params ?? {}, absolute, (window as any).Ziggy) }
  catch { return '#' }
}

/** Sidebar states */
const sidebarOpen    = ref(true)   // mobile
const sidebarCompact = ref(false)  // desktop compact
onMounted(() => {
  const v = localStorage.getItem('bko.sidebar');            if (v) sidebarOpen.value    = v === '1'
  const c = localStorage.getItem('bko.sidebar.compact');    if (c) sidebarCompact.value = c === '1'
})
watch(sidebarOpen,    v => localStorage.setItem('bko.sidebar', v ? '1' : '0'))
watch(sidebarCompact, v => localStorage.setItem('bko.sidebar.compact', v ? '1' : '0'))
...
const paletteQ = ref('')
const paletteList = computed(() => items.value
  .filter(i => i.enabled && hasRoute(i.route))
...
const keyMap: Record<string, string> = {
  d:'admin.dashboard', p:'admin.projects.index', a:'admin.posts.index', q:'admin.quotes.index',
  r:'admin.rfps.index', c:'admin.contacts.index', u:'admin.users.index', s:'admin.settings.index',
}
let awaitingG = false
const onKeydown = (e: KeyboardEvent) => {
  ...
  if (awaitingG) {
    const routeName = keyMap[e.key.toLowerCase()]
    if (routeName && hasRoute(routeName)) { e.preventDefault(); awaitingG = false; router.visit(href(routeName)) }
  }
}
...
<template>
  <div class="min-h-screen bg-bk-night text-bk-off flex">
    ...
    <aside
      :class="[
        'fixed md:static z-40 inset-y-0 left-0 p-4 border-r border-white/10 backdrop-blur transform transition-transform',
        'bg-[radial-gradient(1200px_600px_at_-10%_-20%,rgba(220,193,118,.08),transparent),linear-gradient(180deg,rgba(255,255,255,.06),rgba(255,255,255,.02))]',
    ...
    >
      ...
      <div class="flex items-center justify-between h-12 px-2">
        <div class="flex items-center gap-2">
          ...
          <div v-if="!sidebarCompact" class="font-display text-xl">
            BKO <span class="text-bk-gold">Construction</span>
          </div>
        </div>
        <div class="flex items-center gap-2">
          <span v-if="!sidebarCompact" class="text-[10px] rounded px-1.5 py-0.5 ring-1 ring-white/15 text-white/70 bg-white/5">CONSTRUCTION</span>
          <button type="button"
      ...
      <div v-if="!sidebarCompact" class="mt-6 text-[11px] text-white/45 px-2">
        © {{ new Date().getFullYear() }} BKO Construction · Back-office
      </div>
    </aside>
          ...
          <details class="group">
            ...
            <div class="absolute right-4 mt-2 w-72 rounded-xl bg-[#0f141a]/95 ring-1 ring-white/15 backdrop-blur shadow-2xl p-2">
              <div class="px-3 py-2 text-xs text-white/70">
                <div class="flex items-center gap-2">
                  <span class="inline-flex items-center gap-1 rounded px-1.5 py-0.5 ring-1 ring-white/15"
                        :class="user?.two_factor_enabled ? 'text-emerald-300 bg-emerald-500/10 ring-emerald-500/30' : 'text-white/70 bg-white/5'">
                    <svg viewBox="0 0 24 24" class="w-3.5 h-3.5"><path fill="currentColor" d="M12 1l9 4v6c0 5-3.8 9.7-9 11-5.2-1.3-9-6-9-11V5l9-4z"/></svg>
                    2FA {{ user?.two_factor_enabled ? 'activée' : 'non active' }}
                  </span>
                  <span class="opacity-70">Dernière connexion :</span>
                  <span class="font-medium">{{ user?.last_login_at ?? '—' }}</span>
                </div>
                <div class="mt-1 flex flex-wrap gap-1" v-if="user?.roles?.length">
                  <span v-for="r in user.roles" :key="r" class="text-[11px] rounded px-1.5 py-0.5 ring-1 ring-white/15 bg-white/5">{{ r }}</span>
                </div>
              </div>
              ...
              <!-- plus de <form> ici : on poste via Inertia -->
              <div class="px-2 pt-2 border-t border-white/10 mt-2">
                <button type="button"
                        class="w-full text-left flex items-center gap-2 px-1.5 py-2 rounded-lg hover:bg-white/10"
    ...
    <div v-if="paletteOpen" class="fixed inset-0 z-[60] bg-black/60 backdrop-blur-sm" @click="paletteOpen=false">
      <div class="max-w-xl mx-auto mt-28 px-4" @click.stop>
        <div class="rounded-2xl overflow-hidden ring-1 ring-white/15 bg-[#0f141a]/95">
          <div class="relative">
            <input id="cmdk" v-model="paletteQ" placeholder="Aller à… (ex: Projets, Devis, Actualités…) — ESC pour fermer"
            ...
            <button type="button" class="absolute right-3 top-1/2 -translate-y-1/2 text-white/60 hover:text-white"
                    @click="paletteOpen=false" aria-label="Fermer">✕</button>
          </div>
          <div class="max-h-80 overflow-y-auto divide-y divide-white/10">
            <button v-for="i in paletteList" :key="i.route"
                    type="button"
              ...
              <span class="inline-flex w-5 justify-center">
                <!-- icons reused -->
                <svg v-if="i.icon==='dash'" viewBox="0 0 24 24" class="w-4 h-4"><path fill="currentColor" d="M3 13h8V3H3v10zm0 8h8v-6H3v6zm10 0h8V11h-8v10zm0-18v6h8V3h-8z"/></svg>
                <svg v-else-if="i.icon==='projects'" viewBox="0 0 24 24" class="w-4 h-4"><path fill="currentColor" d="M3 5h18v2H3V5zm0 6h18v2H3v-2zm0 6h12v2H3v-2z"/></svg>
                <svg v-else-if="i.icon==='quotes'" viewBox="0 0 24 24" class="w-4 h-4"><path fill="currentColor" d="M21 6h-8l-2-2H3v16h18V6z"/></svg>
                <svg v-else-if="i.icon==='rfps'" viewBox="0 0 24 24" class="w-4 h-4"><path fill="currentColor" d="M19 3H5a2 2 0 00-2 2v14l4-4h12a2 2 0 002-2V5a2 2 0 00-2-2z"/></svg>
                <svg v-else-if="i.icon==='contacts'" viewBox="0 0 24 24" class="w-4 h-4"><path fill="currentColor" d="M12 12a5 5 0 100-10 5 5 0 000 10zm-9 9a9 9 0 1118 0H3z"/></svg>
                <svg v-else-if="i.icon==='posts'" viewBox="0 0 24 24" class="w-4 h-4"><path fill="currentColor" d="M4 4h16v2H4V4zm0 5h16v2H4V9zm0 5h10v2H4v-2z"/></svg>
                <svg v-else-if="i.icon==='users'" viewBox="0 0 24 24" class="w-4 h-4"><path fill="currentColor" d="M16 11a4 4 0 10-8 0 4 4 0 008 0zM4 20a8 8 0 1116 0H4z"/></svg>
                <svg v-else-if="i.icon==='settings'" viewBox="0 0 24 24" class="w-4 h-4"><path fill="currentColor" d="M19.14 12.94a7.07 7.07 0 000-1.88l2.03-1.58a.5.5 0 00.12-.64l-1.92-3.32a.5.5 0 00-.6-.22l-2.39.96a7.1 7.1 0 00-1.62-.94l-.36-2.54a.5.5 0 00-.5-.42h-3.84a.5.5 0 00-.5.42l-.36 2.54c-.58-.23 1.12-.54 1.62-.94l-2.39-.96a.5.5 0 00-.6.22L2.7 8.84a.5.5 0 00.12.64l2.03 1.58a7.07 7.07 0 000 1.88L2.82 14.6a.5.5 0 00-.12.64l1.92 3.32a.5.5 0 00.6.22l2.39-.96c.5.4 1.04.71 1.62.94l.36 2.54a.5.5 0 00.5.42h3.84a.5.5 0 00.5-.42l.36-2.54c.58-.23 1.12-.54 1.62-.94l2.39.96a.5.5 0 00.6-.22l1.92-3.32a.5.5 0 00-.12-.64l-2.03-1.66zM12 15a3 3 0 110-6 3 3 0 010 6z"/></svg>
                <svg v-else-if="i.icon==='security'" viewBox="0 0 24 24" class="w-4 h-4"><path fill="currentColor" d="M12 1l9 4v6c0 5-3.8 9.7-9 11-5.2-1.3-9-6-9-11V5l9-4zM7 11h10v2H7v-2z"/></svg>
              </span>
              <span class="text-sm">{{ i.label }}</span>
              <span class="ml-auto text-[11px] text-white/40">{{ i.route }}</span>
            </button>
            <div v-if="!paletteList.length" class="px-4 py-6 text-sm text-white/60">
              Aucune correspondance. Tapez “Projets”, “Devis”, “Paramètres”, …
            </div>
          </div>
        </div>
      </div>
